...
describe 'admin can view last game results on league#show sidebar', type: :feature do
  ...
  describe 'when there is a last game available' do
    let!(:first_game) { create(:game, completed: true, date: 2.days.ago, season: season).decorate }
    let(:last_game) { create(:game, completed: true, date: Date.today, season: season).decorate }
    ...
    it 'shows last game results on sidebar' do
      ...
      within('.season-show__game-results') do
